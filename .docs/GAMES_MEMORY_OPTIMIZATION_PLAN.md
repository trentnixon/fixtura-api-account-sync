...
if (isLargeAssociation && !this.options.isolateByCategory) {
  logger.info(
    "[GAMES] Large association detected - auto-enabling category isolation"
  );
...
if (this.options.memoryTracker) {
  ...
  if (stats.rss > 500 * 1024 * 1024) {
    // > 500MB
    batchSize = Math.max(1, Math.floor(batchSize * 0.5)); // Reduce by 50%
    logger.warn(
      `[GAMES] Memory is high (${stats.rss.toFixed(
        2
      )} MB) - reducing batch size to ${batchSize}`
    );
  }

  // Increase batch size if memory is low
  if (stats.rss < 300 * 1024 * 1024) {
    // < 300MB
