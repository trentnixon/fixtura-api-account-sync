...
async function ScrapeChildren(matchElement, SELECTOR) {
  const gameCount = await matchElement.$$eval(
    SELECTOR,
    (games) => games.length
  );
...
const ScrapeStatus = async (matchElement, SELECTORS) => {
  const statusFinalSelector = await Find_Item(matchElement, SELECTORS);
...
async function scrapeTypeTimeGround(matchElement, BaseXpath) {
  const parentDivSelector = `${BaseXpath} > div > div:nth-child(2)`;
  ...
  const typeSelector = `${BaseXpath} > div > div:nth-child(2) > span:nth-child(${
    1 + offset
  })`;
  const timeSelector = `${BaseXpath} > div > div:nth-child(2) > span:nth-child(${
    2 + offset
  }) > div:nth-child(2) > span`;
  const groundSelector = `${BaseXpath} > div > div:nth-child(2) > span:nth-child(${
    3 + offset
  }) span`;
