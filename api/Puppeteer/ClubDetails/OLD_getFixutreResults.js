...
class getFixtureResults {
  ...
  async Setup(ClubTeams) {
    const page = await this.browser.newPage();
    let teamIndex = 0;

    try {
      for (const { id, attributes: team } of ClubTeams.data) {
  ...
  async getGameDetails(page) {
    try {
      const details = await page.evaluate(() => {
        ...
        const status = headerEl ? headerEl.querySelector("span")?.textContent.trim() : "";
        ...
        const competitionLink = typeEl
          ...
          const tossResult = tossEl ? tossEl.querySelector(".gXUjpw")?.textContent.trim() : "";
